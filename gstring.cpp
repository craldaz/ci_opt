...
void GString::String_Method_Optimization()
{
  ...
  if (isFSM>0)
    ...
    cout << "****** Starting IC-SSM calculation *****" << endl;
		...
		cout << "****** Starting IC-ESM calculation *****" << endl;
...
// create union_ic
  ...
  for (int i=0;i<nnmax;i++)
	...
	double** dgrada = new double*[nnmax]; 
	for (int i=0;i<nnmax;i++)
	...
	double** dgradqa = new double*[nnmax]; 
	for (int i=0;i<nnmax;i++)
			dgradqa[i] = new double[len_d]; 
	for (int n=0;n<nnmax;n++)
		for (int i=0;i<len_d;i++)
			dgradqa[n][i]=0.;

	double** dvecqa = new double*[nnmax]; 
	for (int i=0;i<nnmax;i++)
			dvecqa[i] = new double[3*natoms];
	for (int n=0;n<nnmax;n++)
		for (int i=0;i<3*natoms;i++)
			dvecqa[n][i] = 0.;

	double** dvecqa_U = new double*[nnmax]; 
	for (int i=0;i<nnmax;i++)
			dvecqa_U[i] = new double[3*natoms];
	for (int n=0;n<nnmax;n++)
		for (int i=0;i<3*natoms;i++)
			dvecqa_U[n][i] = 0.;

double** dgradqa_U = new double*[nnmax]; 
	for (int i=0;i<nnmax;i++)
			dgradqa_U[i] = new double[size_ic]; 
	for (int n=0;n<nnmax;n++)
...
//cartesian rotation, incomplete
...
#if 0
	if (restart_wfn)
		newic.grad1.grads(coords[0], grads[0], icoords[0].Ut, 3); //restarting from wfu, need to calculate grads
#endif
#if !USE_MOLPRO
  ...
  for (int n=0;n<nnmax0;n++)
  {
    ...
    if (isRestart) icoords[n].grad1.seedType = 3;
		...
		if (n==0 && restart_wfn) icoords[n].grad1.seedType = 3; 		//skip seeding or copying because aldready have wfn
		if ((isSSM || isESSM) && (n==nnmax0-1))
				icoords[n].grad1.seedType = 3; //skip seeding or copying because single-ended
 	  icoords[n].grad_init(infile0,ncpu,runNum,runend+n,0);
		if (restart_wfn && n==0) V0=icoords[n].grad1.grads(coords[0], grads[0], icoords[0].Ut, 3); //restarting from wfu, seedType=3,need to  calculate E and grads
...
//exit(-1);
 ...
 //after IC's are ready
  ...
  {
#if USE_MOLPRO
		//V0 = icoords[0].grad1.E[0]; //Already been calculated above
#else
    V0 = grad1.grads(coords[0], grads[0], icoords[0].Ut, 3);
#endif
  }
  V_profile[0] = 0.;
  if (!isSSM && !isRestart && !isESSM) 
  {
  ...
  if (isSSM || isESSM)
    printf(" at beginning, starting V is 0.0 (%8.6f) \n",V0/627.5);
  else
    printf(" at beginning, starting V's are %8.6f %8.6f \n",V_profile[0],V_profile[nnmax-1]);
  ...
  if (!isRestart)
  {
    	...
    	starting_string(dq,3);
			...
			starting_dstring(3,dqa,dqmaga,dgrada,dgradqa,dgradqa_U,dveca,dvecqa,dvecqa_U,grads);
    else if (!isSSM && !isESSM)   //doesn't allow for FSM currently
      starting_string(dq,4);
    active[1] = active[nnmax-2] = 1;
    if (isSSM || isESSM) active[nnmax-2] = -1;
    if (GROWD==1 && !isSSM && !isESSM) 
    {
      printf(" removing product node (GROWD==1) \n");
      nnP = 1; active[nnmax-2] = -1; nn--;  
    }
    if (GROWD==2 && !isSSM && !isESSM)
    {
 ...
 //new growth loop in here
  if (!isRestart)
    growth_iters(max_iter,totalgrad,gradrms,endenergy,strfileg,tscontinue,gaddmax,osteps,oesteps,dqa,dqmaga,ictan,dgrada,dgradqa,dgradqa_U,dveca,dvecqa,dvecqa_U,grads);
    ...
    if (tagname=="SM_TYPE") {
      ...
      else if (tok_line[1]=="SSMFR")
      {
        ...
        hessSSM = 1;
 			...
 			else if (tok_line[1]=="ES-SSM")
      {
        printf("  -using Excited State SM \n");
        isFSM = 0;
        isSSM = 0;
				isESSM = 1; 
      }
...
void GString::calc_dgradq(double* dgrad, double* dgradq,int run,int rune)
{

#if 0
	newic.grad1.diffgrad(newic.coords,dgrad,run,rune); 
#else
	//grad1.grads(coords,grad,Ut,1);
	//printf(" Calculating dgrad\n");
	//for (int i=0;i<3*natoms;i++)	
	//	dgrada= grad1.grada[wstate2-1][i]- grad1.grada[wstate-1][i];
	
#endif
	newic.dgrad_to_q(dgrad,dgradq);
	//newic.project_dgrad(dgradq);
	//newic.bmat_create();
	return;
}
...
void GString::starting_dstring(int nnodes, double** dqa, double* dqmaga,double** dgrada, double** dgradqa,double** dgradqa_U,\
		double** dveca,double** dvecqa,double** dvecqa_U,double** grads)
{
	//printf(" starting dstring\n");
  int nbonds = newic.nbonds;
  int nangles = newic.nangles;
  int ntor = newic.ntor;
  int size_ic = newic.nbonds+newic.nangles+newic.ntor;
  ...
  for (int i=0;i<nstates-1;i++)
  {
	 ...
	 printf(" dE[%i]:  %5.4f\t ",i+1,icoords[0].grad1.dE[i]);
  }
  printf("\n");
  double dE = icoords[0].grad1.dE[wstate2-2]/627.5; //kcal2Hartree
#if 0
	printf(" Grads\n");
	for (int j=0;j<2;j++)
	{for (int i=0;i<3*natoms;i++)	
		printf(" %1.3f",icoords[0].grad1.grada[j][i]);
		printf(" \n");
	}
#endif
	printf(" Calculating dgrad[0]\n");
	for (int i=0;i<3*natoms;i++)	
		dgrada[0][i]= icoords[0].grad1.grada[1][i]- icoords[0].grad1.grada[0][i];
	printf(" Calculating dvec[0]\n");
	icoords[0].grad1.dvec_calc(icoords[0].coords,dveca[0],1,0);

	printf(" Copying dgrada[0] to dgrada[1]\n");
	for (int i=0;i<3*natoms;i++)	
		dgrada[1][i] = dgrada[0][i];
	printf(" Copying dveca[0] to dveca[1]\n");
	for (int i=0;i<3*natoms;i++)	
		dveca[1][i] = dveca[0][i];

	printf(" Copying grads[0] to grads[1]\n");
	for (int i=0;i<3*natoms;i++)	
		grads[1][i]= grads[0][i];
	printf(" Copying icoords[0].grad1.E  to icoords[1].grad1.E\n");
	for (int i=0;i<nstates;i++)
		icoords[1].grad1.E[i] = icoords[0].grad1.E[i];
#if 0
	printf(" printing dgrad\n");
	for (int i=0;i<3*natoms;i++)
		printf("%1.3f\t",dgrada[0][i]);	
	printf("\n");
#endif
#if 0
	double test=0;
	for (int i=0;i<3*natoms;i++)
		test+=dgrada[0][i]*dgrada[0][i];
	test=sqrt(test);
	printf(" norm dgrad cartesians %1.2f\n",test);
#endif

	icoords[0].dgrad_to_q(dgrada[0],dgradqa[0]);
	icoords[0].dgrad_to_q(dveca[0],dvecqa[0]);

// Form the initial string

#if 0	
	string nstr = StringTools::int2str(iR,4,"0");
	string dfile = "scratch/dgrad_o"+nstr+".molden";
	printf(" Printing dgrad to file %s\n",dfile.c_str());
	icoords[0].print_dgrad_save(dfile,newic.coords,dgrada[iR]);
#endif

		double norm_dg=0.; 
#if !DG_ROT
	norm_dg=icoords[0].project_gradq(dgradqa[iR],dgradqa_U[iR]);
	icoords[0].project_gradq(dvecqa[0],dvecqa_U[0]);
#else DG_ROT
	norm_dg=icoords[0].dgrot_mag(dgradqa[0],dvecqa[0]);
	icoords[0].project_gradq(dvecqa[0],dvecqa_U[0]);
	icoords[0].project_gradq(dgradqa[0],dgradqa_U[0]);
#endif


#if 0
    printf(" printing dgradqa_U[0] \n");
	for (int j=0;j<size_ic;j++)
		printf(" %1.3f ",dgradqa_U[0][j]);
    printf("\n");
#endif

	icoords[0].constrain_bp(dgradqa_U[0],dvecqa_U[0]);
	icoords[0].print_q();
	icoords[0].bmat_create();
	icoords[0].make_Hint();
		
    icoords[1].reset(natoms,anames,anumbers,icoords[0].coords);
    //com_rotate_move(iR,iP,iN,1.0); //operates on iN via newic
    icoords[1].bmatp_create();
    icoords[1].bmatp_to_U();
    icoords[1].bmat_create();
 ...
 icoords[1].make_Hint();
      ...
      printf(" copying Hessian from node %i \n",0);
      for (int i=0;i<size_ic*size_ic;i++)
        icoords[1].Hintp[i] = icoords[0].Hintp[i];
      icoords[1].newHess = 2;
#endif
	nnR++;
  return;
...
int GString::addDNode(int n1,int n2,int n3,double** dqa, double* dqmaga,double** dgrada, double** dgradqa,double** dgradqa_U,double** dveca,double** dvecqa,double** dvecqa_U,double** grads)
{
	//printf(" in addDNode\n"); 
  printf(" adding dnode: %i between %i %i \n",n2,n1,n3);
  if (n1==n2 || n2==n3 || n1==n3)
  {
    printf(" cannot add node, exiting \n"); 
    exit(1);
  ...
  int size_ic = newic.nbonds+newic.nangles+newic.ntor;
  int len_d = newic.nicd0;
	//double* dgradq0 = new double[len_d+100]; 
  int nstates = icoords[0].grad1.nstates;
  int wstate = icoords[0].grad1.wstate;
	int wstate2 = icoords[0].grad1.wstate2;
	if (wstate2==0)
	...
	printf(" printing dgrad\n");
	for (int i=0;i<3*natoms;i++)
		printf("%1.3f\t",dgrada[n1][i]);	
#endif
#if 0
	double test=0;
	for (int i=0;i<3*natoms;i++)
		test+=dgrada[n1][i]*dgrada[n1][i];
	test=sqrt(test);
	printf(" norm dgrad cartesians %1.2f\n",test);
#endif
//	newic.dgrad_to_q(dgrada[n1],dgradqa[n1]);
#if 0 
	string nstr = StringTools::int2str(n1,4,"0");
	string dfile = "scratch/dgrad_o"+nstr+".molden";
	printf(" printing dfile %s\n",dfile.c_str());
	newic.print_dgrad_save(dfile,icoords[n1].coords,dgrada[n1]);
#if DG_ROT
	string dcfile = "scratch/dvec_o"+nstr+".molden";
	printf(" printing dfile %s\n",dfile.c_str());
	newic.print_dgrad_save(dcfile,icoords[n1].coords,dveca[n1]);
#endif
#endif
  icoords[n2].reset(natoms,anames,anumbers,icoords[n1].coords);
	printf(" Copying dgrada[%i] to dgrada[%i]\n",n1,n2);
	for (int i=0;i<3*natoms;i++)	
		dgrada[n2][i] = dgrada[n1][i];
	printf(" Copying dveca[%i] to dveca[%i]\n",n1,n2);
	for (int i=0;i<3*natoms;i++)	
		dveca[n2][i] = dveca[n1][i];

	printf(" Copying grads[%i] to grads[%i]\n",n1,n2);
	for (int i=0;i<3*natoms;i++)	
		grads[n2][i]= grads[n1][i];
	printf(" Copying icoords[%i].grad1.E  to icoord[%i].grad1.E\n",n1,n2);
	for (int i=0;i<nstates;i++)
		icoords[n2].grad1.E[i] = icoords[n1].grad1.E[i];

#if 0
	  icoords[n2].make_Hint();
#else
    for (int i=0;i<size_ic*size_ic;i++)
      icoords[n2].Hintp[i] = icoords[n1].Hintp[i];
    icoords[n2].newHess = 2;
#endif
    active[n2] = 1;
...
void GString::opt_steps_d(double** dgrada, double** dgradqa, double** dgradqa_U,double** dveca,\
double** dvecqa,double** dvecqa_U, int osteps,double** grads)
{
  printf("\n"); fflush(stdout);
  ...
  for (int n=n0+1;n<nnmax;n++)
  {
    ...
    {	
      	string nstr = StringTools::int2str(n,2,"0");
				//icoords[n].constrain_gradq(dgradqa_U[n]);
        V_profile[n] = icoords[n].opt_D("scratch/xyzfile.xyz"+nstr,osteps,n,dgrada[n],dgradqa[n],dgradqa_U[n],dveca[n],dvecqa[n],dvecqa_U[n],grads[n]);
    } 
  } //loop over opt
  printf("\n");
  for (int n=n0+1;n<nnmax-1;n++)
...
void GString::get_tangents_1gh(double** dqa, double* dqmaga, double** ictan,double** dgrada, double** dgradqa,double** dgradqa_U)
{
  int nbonds = newic.nbonds;
  int nangles = newic.nangles;
  int ntor = newic.ntor;
...
if (nnR==2) //calculate the dgrad on the reactant
		{
    	newic.reset(natoms,anames,anumbers,icoords[0].coords);
    	newic.bmatp_create();
    	newic.bmatp_to_U();
			newic.bmat_create();
			calc_dgradq(dgrada[0],dgradqa[0],1,0); //CRA fix run later
			newic.project_gradq(dgradqa[0],dgradqa_U[0]);
		}
  ...
  for (int n=0;n<ncurrent;n++)
  {
    ...
    if (isSSM && nlist[2*n]==nnR-1)
		{
      tangent_1b(ictan[nlist[2*n]]);
			newic.reset(natoms,anames,anumbers,icoords[nlist[2*n]].coords);
			newic.update_ic();
    	newic.bmatp_create();
    	newic.bmatp_to_U();
			newic.bmat_create();
			calc_dgradq(dgrada[nlist[2*n]],dgradqa[nlist[2*n]],1,nlist[2*n]); //CRA fix run later
			newic.project_gradq(dgradqa[nlist[2*n]],dgradqa_U[nlist[2*n]]); 
    	newic.reset(natoms,anames,anumbers,icoords[nlist[2*n+1]].coords);
		}
    else
      tangent_1(ictan[nlist[2*n]]);
    ...
    //newic.bmat_create();
		...
		if (nlist[2*n]==nnR-1)
		{
			string nstr = StringTools::int2str(nlist[2*n],4,"0");
			string dfile = "scratch/dgrad_i"+nstr+".molden";
			newic.print_dgrad_save(dfile,icoords[nlist[2*n]].coords,dgrada[nlist[2*n]]);

			double ovlap_dgrad = 0;
			for (int i=0;i<size_ic;i++)
				ovlap_dgrad += dgradqa_U[nlist[2*n]][i]*ictan[nlist[2*n]][i];
			printf("initial dot(dgradq_U[%i],ictan[%i]) = %2.3f\n",nlist[2*n],nlist[2*n],ovlap_dgrad); 
		}
		if (nlist[2*n]==nnR-2)
		{
			string nstr = StringTools::int2str(nlist[2*n],4,"0");
			string tfile = "scratch/tangent_u"+nstr+".molden";
			newic.print_tangent(tfile,icoords[nlist[2*n]+1].coords,icoords[nlist[2*n]].coords);
			double ovlap_dgrad = 0;
			for (int i=0;i<size_ic;i++)
				ovlap_dgrad += dgradqa_U[nlist[2*n]][i]*ictan[nlist[2*n]][i];
			printf("updated dot(dgradq_U[%i],ictan[%i]) = %2.3f\n",nlist[2*n],nlist[2*n],ovlap_dgrad); 
		...
		if (nlist[2*n]==nnR-3)
		{
			...
			string nstr = StringTools::int2str(nlist[2*n],4,"0");
			string tfile = "scratch/tangent_f"+nstr+".molden";
			newic.print_tangent(tfile,icoords[nlist[2*n]+1].coords,icoords[nlist[2*n]].coords);
			string dfile = "scratch/dgrad_f"+nstr+".molden";
			newic.print_dgrad_save(dfile,icoords[nlist[2*n]].coords,dgrada[nlist[2*n]]);

			double ovlap_dgrad = 0;
			for (int i=0;i<size_ic;i++)
				ovlap_dgrad += dgradqa_U[nlist[2*n]][i]*ictan[nlist[2*n]][i];
			printf("optimized dot(dgradq_U[%i],ictan[%i]) = %2.3f\n",nlist[2*n],nlist[2*n],ovlap_dgrad); 
		}
 ...
 for (int n=0;n<ncurrent-1;n++)
  {
    printf(" printing dgradqa_U[%i] \n",nlist[2*n]);
		for (int j=0;j<size_ic;j++)
			printf(" %1.3f ",dgradqa_U[nlist[2*n]][j]);
    printf("\n");
  }
#endif
...
//  string xyzstring = "xyzfile.xyzc";
  ...
  if (active[n]>-1 || active[n]==-2 || isFSM || isSSM || isESSM)
  {
	...
	xyzfilec << "max-step" <<endl; //not really max-step
	for (int n=0;n<nodes;n++)
		xyzfilec<< icoords[n].grad1.dE[icoords[n].grad1.wstate2-2] << endl;
...
void GString::growth_iters(int max_iter, double& totalgrad, double& gradrms, double endenergy, \
		string strfileg, int& tscontinue, double gaddmax, int osteps, int oesteps, double** dqa, \
		double* dqmaga, double** ictan, double** dgrada, double** dgradqa, double** dgradqa_U,\
		double** dveca,double** dvecqa,double** dvecqa_U,double** grads)
{
  double rn3m6 = sqrt(3*natoms-6);
  int nmax;
  ...
  if ((ngrowth>4 && tstype!=2) || (ngrowth>14 && tstype==2))
  {
    ...
    exit(1);
		...
		printf(" TURNING OFF SSM GROWTH TERMINATIONS\n");
//  for (int n=0;n<nnmax0;n++) icoords[n].grad1.knnr_active = 3;
  ...
  for (;oi<max_iter;oi++)
  {
	...
	if (nnR >= 3 && !isESSM)
		{
			printf(" Switching to Following Dgrad!");
			isESSM = 1; 
			isSSM = 0;
			icoords[nnR].grad1.wstate2=icoords[nnR].grad1.wstate;
			icoords[nnR].grad1.wstate = icoords[nnR].grad1.wstate2-1;  
			osteps=3;
			printf("wstate2 = %i\n",icoords[nnR].grad1.wstate2);  
		}
    if (oi>0 && isFSM)
      set_fsm_active(nnR,nnmax-nnP-1);
    if (oi>0 && (isSSM || isESSM))
		{		
      set_fsm_active(nnR,nnR);
		}
    if((icoords[nnR-1].gradrms<gaddmax && GROWD!=2) || isFSM || isSSM || isESSM)
    {
      if (oi>0 && nn < nnmax)
      {
				if (!isESSM)
        	addednode = addNode(nnR-1,nnR,nnmax-nnP);
				if (isESSM)
        	addednode = addDNode(nnR-1,nnR,nnmax-nnP,dqa,dqmaga,dgrada,dgradqa,dgradqa_U,dveca,dvecqa,dvecqa_U,grads);
        if (addednode) nnR++;
        else
        {
    ...
    if (!isFSM && !isSSM && !isESSM)
      ic_reparam_g(dqa,dqmaga);

		
	 	if (!isESSM){
    	get_tangents_1gh(dqa,dqmaga,ictan,dgrada,dgradqa,dgradqa_U); //Hybrid
		opt_steps(dqa,ictan,osteps,oesteps);}

#if 1
		if (isESSM)
			opt_steps_d(dgrada,dgradqa,dgradqa_U,dveca,dvecqa,dvecqa_U,osteps,grads);
#endif 
#if 0
	 	if (!isESSM)
		{
...
//        n0 = find_peaks(3) - 3; //put 2 free nodes prior to TS
    ...
    for (int i=1;i<nnmax-1;i++)
		{
    	if ((active[i]>-1 || isSSM || isFSM) && !isESSM)
    	{
      	if (icoords[i].gradrms<1000.)
      	{
			...
			if (active[i]>1 && isESSM)
			{
				gradrms=icoords[i].gradrms;
				totalgrad = icoords[i].gradrms*rn3m6;
			}
		}
    if (isSSM)
    ...
    for (int i=0;i<nnmax;i++)
    {
		...
		for (int n=0;n<nnR;n++)
		{
			...
			for (int i=0;i<nstates-1;i++)
			{
				...
				printf(" dE[%i]:  %5.4f\t ",i+1,icoords[n].grad1.dE[i]); 
			}
		}
#endif
...
//    print_string_clump(nnmax,totalgrad,allcoords,strfile);
		...
		printf("\n Checking if ES-SM is finished\n");
		for (int n=0;n<nnR;n++)
		{
			if (gradrms<CONV_TOL && icoords[n].grad1.dE[icoords[n].grad1.wstate2-2]< 0.01)
				{
					printf(" gradrms < CONV_TOL && dE[%i]<0.01\n",icoords[n].grad1.wstate2-2);
					exit(-1);
				}	
		}//ES-SSM growth terminations
