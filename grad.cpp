...
//  printf(" gqce"); fflush(stdout);
  ...
  if (gradcalls==0 && seedType<1)
  {
    ...
    string cmd = "cp scratch/mp_"+runNameCopy+" scratch/mp_"+runName0;
		...
		//printf("temporarily enabling this feature\n");
    //printf("  via: %s \n",cmd.c_str());
    system(cmd.c_str());
  }
...
{ 
  ...
  runName0 = StringTools::int2str(runNum,4,"0")+"."+StringTools::int2str(runend,4,"0");
	...
	int success = mp1.calc_dvec();
  int error = mp1.getDVec(dvec);
#else
...
int Gradient::diffgrad(double* coords, double* dgrad,int run,int rune)
{ 
...
#if USE_MOLPRO
	mp1.runname("mp_"+runName0);
	int success = mp1.calc_dgrad();
  int error = mp1.getdGrad(dgrad);
#else
	printf(" not yet implemented \n");
	return 1;
#endif
...
int Gradient::read_nstates()
{
  ...
  if (nstates0<0)
  {
	...
	qchemsf1.wstate=wstate; 
	qchemsf1.wstate2=wstate2; 
	qchemsf1.wstate3=wstate3;
  return nstates0;
}
