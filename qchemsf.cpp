...
//    cout << " RR " << line << endl;
...
#endif
#if EOM 
		if (wstate>1 && nf==0)  //because not calculating gs force 
			{
				if (line.find("CCSD T converged.")!=string::npos)
					{ 
      			tok_line = StringTools::tokenize(line, " \t");
      			energy=atof(tok_line[0].c_str());
      			E[nf]=atof(tok_line[0].c_str());
						cout <<"the energy of gs is " <<  E[nf] << endl; 
						nf++;
					}
			}  
    ...
    if (line.find("Energy read as")!=string::npos)
    {
      ...
      E[nf]=atof(tok_line[3].c_str());
			//cout <<"the energy of state is " <<  E[nf] << endl; 
      nf++;
    }
