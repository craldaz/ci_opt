...
//  if (isTSnode) SCALEQN = 2.;
  ...
  double energy = grad1.grads(coords, grad, Ut, 1) - V0;
	//This is wasteful because it's already been calc
  energyp = energy;
  energyl = energy;
#if 0
  ofstream xyzfile;
...
double ICoord::opt_D(string xyzfile_string, int nsteps, int n, double* dgrad,double* dgradq,\
 double* dgradq_U, double* dvec, double* dvecq, double* dvecq_U,double* grads)
{
	printf(" In opt_D\n");
	//printf(" oc"); fflush(stdout);
  printout = "";
  ...
  noptdone = 1;
	...
	printf(" Average energy: %3.3f\n",energy);
	for (int i=0;i<3*natoms;i++)
		grad[i]=grads[i];

#if 0
	printf(" printing dgrad[%i]\n",n);
	for (int i=0;i<3*natoms;i++)
		printf("%1.3f\t",dgrad[i]);	
	printf("\n");
	printf(" printing dvec[%i]\n",n);
	for (int i=0;i<3*natoms;i++)
		printf("%1.3f\t",dvec[i]);	
#endif
#if 0
	string nstr = StringTools::int2str(n,4,"0");
	string dfile = "scratch/dgrad_i"+nstr+".molden";
	printf(" Printing dgrad to file %s\n",dfile.c_str());
	print_dgrad_save(dfile,coords,dgrad);
	string dcfile = "scratch/dvec_i"+nstr+".molden";
	printf(" Printing dvec to file %s\n",dcfile.c_str());
	print_dgrad_save(dcfile,coords,dvec);
#endif

	dgrad_to_q(dgrad,dgradq);
	dgrad_to_q(dvec,dvecq);
	double norm_dg=0.;
#if DG_ROT
	norm_dg=dgrot_mag(dgradq,dvecq);
	project_gradq(dvecq,dvecq_U);
	project_gradq(dgradq,dgradq_U);
#else
	project_gradq(dvecq,dvecq_U);
	norm_dg=project_gradq(dgradq,dgradq_U);
	printf(" norm_dg = %1.2f",norm_dg); 
#endif

	constrain_bp(dgradq_U,dvecq_U);
	bmat_create();
	print_q();
  Hintp_to_Hint();
  ...
  for (int n=0;n<OPTSTEPS;n++)
  {
    ...
    for (int i=0;i<natoms;i++) 
    {
      ...
      xyzfile << endl;
		...
		for (int i=0;i<nstates-1;i++)
		{
			...
			printf(" dE[%i][%i]:  %5.4f\t ",n,i,grad1.dE[i]); 
		}
		printf("\n");
		double deltaE = grad1.dE[wstate2-2]/627.5; //kcal2Hartree
    ...
    energy = grad1.grads(coords, grad, Ut, 1) - V0;
		...
		grad1.dvec_calc(coords,dvec,1,n);
		dgrad_to_q(dgrad,dgradq);
		dgrad_to_q(dvec,dvecq);
#if DG_ROT
		norm_dg=dgrot_mag(dgradq,dvecq);
...
#endif
#else
		printf(" Updating DVEC using Maeda's method\n");
#if DG_ROT
		update_dvec(dvecq,dgradq,dgrad);
		dgrot_mag(dgradq,dvecq);
		project_gradq(dvecq,dvecq_U);
		project_gradq(dgradq,dgradq_U);
#else
		update_dvec(dvecq,dgradq,dgrad);
		dc_rot(dgradq,dvecq); //BP
		project_gradq(dgradq,dgradq_U);
		project_gradq(dvecq,dvecq_U);
#endif
#endif
    ...
    sprintf(sbuff," E(M): %1.2f gRMS: %1.4f",energy,gradrms); printout += sbuff;
    if (gradrms<OPTTHRESH && !bcp && deltaE < 0.01) 
    {
      sprintf(sbuff," * \n"); printout += sbuff;
      break;
    }
...
#endif
#if 1
		if (grad1.dE[wstate2-2] < 0.1 && gradrms<0.01 ) 
		{
      if (dE > 0.001 && !isTSnode)
      {
        sprintf(sbuff," dE>0, decreasing DMAX "); printout += sbuff;
...
int ICoord::dgrad_to_q(double* dgrad,double* dgradq)
{	
 // printf(" in dgrad_to_q \n"); 
...
double ICoord::project_gradq(double* gradq,double* gradq_U)
{
  int len = nbonds+nangles+ntor;
	printf(" Projecting\n"); 
  ...
  printf("\n");
	...
	for (int i=0;i<len;i++)
		for (int j=0;j<nicd0;j++)
			gradq_U[i]+=gradq0[j]*Ut[j*len+i];

#if 0
	norm=0.;
	printf("check if gradq_U is normalized\n");
	for (int i=0;i<len;i++)
		norm += gradq_U[i]*gradq_U[i];
	norm=	sqrt(norm);
	printf("norm: %1.3f\n",norm);
#endif

#if 0
	printf(" Normalized gradq_U vector\n");
	for (int i=0;i<len;i++)
		printf("%1.2f ",gradq_U[i]);
	printf("\n");
...
void ICoord::constrain_bp(double* dgradq_U,double* dvecq_U)
{
  int len = nbonds+nangles+ntor;
#if 0
	for (int i=0;i<len;i++)
		printf(" %1.2f",dgradq_U[i]);
	printf("\n");
	for (int i=0;i<len;i++)
		printf(" %1.2f",dvecq_U[i]);
	printf("\n");
#endif 
	double overlap=0.;
	for (int i=0;i<len;i++)
		overlap+=dvecq_U[i]*dgradq_U[i];
	printf(" Overlap between normalized GD and DC = %1.3f\n",overlap);	
#if !DG_ROT
	for (int i=0;i<len;i++)	
		dvecq_U[i]=dvecq_U[i] - overlap*dgradq_U[i];
#endif
	double norm;
	for (int i=0;i<len;i++)
		norm+= dvecq_U[i]*dvecq_U[i];
	norm=sqrt(norm);
	//printf(" norm of dvecq_U = %1.2f",norm);
	for (int i=0;i<len;i++)
		dvecq_U[i] = dvecq_U[i]/norm;
#if !DG_ROT
	double overlap2 =0.;
	for (int i=0;i<len;i++)
		overlap2 +=dvecq_U[i]*dgradq_U[i];
  ...
  for (int i=0;i<len;i++) 
  for (int j=0;j<len;j++)
    dot_gd[i] += dgradq_U[j]*Ut[i*len+j]; 
  double* dot_dc = new double[len];
  for (int i=0;i<len;i++) dot_dc[i] =0.;
  for (int i=0;i<len;i++) 
  for (int j=0;j<len;j++)
    dot_dc[i] += dvecq_U[j]*Ut[i*len+j]; 
	
  for (int i=0;i<nicd0;i++)
  {
    if (i!=nicd0-1)
    for (int j=0;j<len;j++)
      Ut[i*len+j] -= (dot_gd[i] * dgradq_U[j] + dot_dc[i] * dvecq_U[j]);
	...
	printf(" Last vector in Ut is dgradq_U");
  for (int j=0;j<len;j++)
    Ut[nicd*len+j] = dgradq_U[j];
	nicd--;
#if 1
	printf(" second to last vector in Ut is dgradq_U\n");
	for (int j=0;j<len;j++)
		Ut[nicd*len+j] = dvecq_U[j];
#endif
#if 0
  printf(" printing dgradq_U vs. Ut[nicd*len]\n");
  for (int j=0;j<len;j++)
    printf(" %1.2f/%1.2f\n",dgradq_U[j],Ut[(nicd+1)*len+j]);
#endif
#if 0
  printf(" printing orthonormalized vectors \n");
  for (int i=0;i<nicd0;i++)
...
void ICoord::constrain_gradq(double* gradq_U)
{
	
  int len = nbonds+nangles+ntor;
	printf(" Orthonormalizing coordinates U vs gradq_U\n"); 
...
double ICoord::dgrot_mag(double* dgradq,double* dvecq)
{
	...
	printf(" \noverlap2 = %1.2f",overlap2);
	
	double overlap=0.;
	for (int i=0;i<len;i++)
		overlap+=dgradq[i]*dvecq[i];

	printf(" overlap b4 = %1.3f",overlap);
	...
	printf(" norm_dg after = %1.3f\n",norm_dg);
	
	return norm_dg;
}

void ICoord::dc_rot(double* dgradq,double* dvecq)
{
	printf(" Orthogonalize the dvec wrt dgrad\n");
	int len=nicd0;
	printf(" dgradq\n");
	for (int i=0;i<len;i++)
		printf(" %1.2f",dgradq[i]);
	...
	double overlap2=0.;
	for (int i=0;i<len;i++)
		overlap2+=dgradq[i]*dgradq[i];
	printf(" overlap2 = %1.2f",overlap2);
	...
	for (int i=0;i<len;i++)
		dvecq[i]=dvecq[i] - overlap*dgradq[i]/overlap2;

	overlap=0.;
	for (int i=0;i<len;i++)
		overlap+=dgradq[i]*dvecq[i];
	printf(" overlap after = %1.3f\n",overlap);

	return;
}

void ICoord::update_dvec(double* dvecq,double* dgradq,double* dgrad)
{
	printf(" Save old BP:\n");
	double*	dgradq_old = new double[nicd0]; 
	for (int i=0;i<nicd0;i++)
		dgradq_old[i] = dgradq[i];
	printf(" dgradq_old\n");
	for (int i=0;i<nicd0;i++)
		printf(" %1.2f",dgradq_old[i]);
	printf(" \ndvecq_old\n");
	for (int i=0;i<nicd0;i++)
		printf(" %1.2f",dvecq[i]);

	printf(" Calculate the magnitudes\n");
	double norm_dg=0.;
	double norm_dc=0.;
	for (int i=0;i<nicd0;i++)
		norm_dg+=dgradq_old[i]*dgradq_old[i];		
	for (int i=0;i<nicd0;i++)
		norm_dg+=dgradq_old[i]*dgradq_old[i];		
		
		
	printf(" \nCalculating new dgrad\n");
	for (int i=0;i<3*natoms;i++)	
		dgrad[i]= grad1.grada[1][i]- grad1.grada[0][i];
	...
	double x=0.; // dot(dgradq_{k-1},dgradq_{k})
	double y=0.; // dot(dvecq_{k-1},dgradq_{k})
	
	for (int i=0;i<nicd0;i++)
		x+=dgradq_old[i]*dgradq[i];
	printf(" \nx = %1.2f",x);
	...
	printf(" denominator = %1.2f\n",denominator);

	for (int i=0;i<nicd0;i++)
		dvecq[i] = (y*dgradq_old[i] + x*dvecq[i])/denominator;
	printf(" Updated dvecq\n");
	for (int i=0;i<nicd0;i++)
		printf(" %1.2f",dvecq[i]);
	printf("\n");	
	
	double overlap=0.;
	printf(" Check if new dvecq is orthogonal to dgradq\n");
	for (int i=0;i<nicd0;i++)
		overlap+=dgradq[i]*dvecq[i];
	printf(" Overlap = %1.2f",overlap);
